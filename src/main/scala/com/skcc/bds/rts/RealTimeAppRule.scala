...
object RealTimeAppRule {
  ...
  def main(args: Array[String]) {
    ...
    //set redis info
    val redisPool = new RedisPool("localhost", 6379)
    val jedis = redisPool.getRedisPool()
    val isRuleExist = true
    
    var drl = "D:/00.Source/600.Scala/sparkTest/src/main/resources/rule_event_1.drl"
    System.out.println("Count : " + args.length)
    //set drools drl file info
    if (args.length > 1) {
      drl = args(1)  
      System.out.println("Rule File : " + drl)
    }

    
    var ksession = RuleManager2.getRuleSession(drl)
    ...
    ssc.addStreamingListener(new StreamingListener() {
        ...
        override def onBatchCompleted(batchCompleted: StreamingListenerBatchCompleted): Unit = {
          ...
          if(isUpdate.toBoolean) {
            //ssc.awaitTermination(2000)
            ksession = RuleManager2.reloadRuleSession(drl)
    ...
    if(isRuleExist) {
      ...
      if(args(0) == "cluster") {
        ...
        //var ksession = RuleRunner.makeRuleKieBase(drl)
        //val rule = new RuleManager2()
        
        lines.foreachRDD { rdd => 
          //rdd.collect.foreach { x => RuleManager2.insertFact("key", x) }
          rdd.collect.foreach { x => RuleManager2.insertFact(x) }
        }
        lines.count()
        lines.print()
      }
